    ...
    ILogger<AccessManagementService> logger)
{
    ...
    public async Task<(PaginatedResultDto<List<AuthorizedPartyDto>>? Result, string? Error, int StatusCode)>
        GetAuthorizedPartiesAsync(string altinnToken, string queryString)
    {
        ...
        var result = JsonSerializer.Deserialize<PaginatedResultDto<List<AuthorizedPartyDto>>>(content, JsonOptions);
